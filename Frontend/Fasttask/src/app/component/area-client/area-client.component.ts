...
export class AreaClientComponent implements OnInit {
  ...
  open_modal() {
    this.dialog
      ...
      .subscribe((result) => {
        ...
        if (result) {
          this.crearTablero(nombreTablero, imagen);
        }
      });
  }
  ...
  crearTablero(nombreTablero: string, imagen: string) {
    ...
    this.newTablero.imagen = imagen;
    this.newTablero.fecha_creacion = formatDate(
      this.newTablero.fecha_creacion,
      'yyyy-MM-dd',
      'en'
    );
    this.newTablero.fecha_modificacion = formatDate(
      this.newTablero.fecha_modificacion,
      'yyyy-MM-dd',
      'en'
    );
    this.newTablero.usuarioFk = Number(
      window.sessionStorage.getItem('idUserr')
    );

    this.tableroService.createTablero(this.newTablero).subscribe({
      next: (tablero) => {
        const currentUrl = this.router.url;
        this.router.navigate([currentUrl + '/tablero/', tablero.id], {
